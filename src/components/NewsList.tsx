          ...
          <Link
                ...
                <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          {article.isBreaking && (
                              <span className="bg-destructive text-destructive-foreground text-xs px-1.5 py-0.5 rounded font-bold">LIVE</span>span>
                                  )}
                                  <span className="text-xs text-primary font-medium">{article.category}</span>span>
                        </div>div>
                        <h3 className="text-sm font-semibold text-foreground line-clamp-2 group-hover:text-primary transition-colors">
                          {article.title}
                        </h3>h3>
                  {descStr && (
                            <p className="text-xs text-muted-foreground line-clamp-1 mt-1">{descStr}</p>p>
                        )}
                        <div className="flex items-center gap-2 mt-1.5 text-xs text-muted-foreground">
                                  <Clock className="h-3 w-3" />
                                  <span>{dateStr ? new Date(dateStr).toLocaleString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}</span>span>
                          {article.source && <span className="text-primary/70">· {article.source}</span>span>}
                        </div>div>
                </div>div>
          </Link>Link>
        );
    ...
    return (
          <div style={{ overflowY: 'visible' }}>
                <div className="flex items-center justify-between mb-4">
                        <h2 className="text-xl font-bold text-foreground">{t('latestNews')}</h2>h2>
            ...
            {error && (
                    <div className="text-center py-4 text-destructive text-sm">{error}</div>div>
                )}
