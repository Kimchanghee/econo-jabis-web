...
async function fetchGeminiNews(query: string, apiKey: string): Promise<NewsArticle[]> {
  ...
  const prompt = `당신은 숙련된 경제 전문 기자입니다. 다음 키워드로 오늘 날짜 기준 최신 경제 뉴스 기사를 5개 찾아서 아래 JSON 형식으로 정확히 반환하세요.
...
본문 작성 규칙 (body 필드):
- 반드시 1500자 이상, 8~10개 문단으로 구성
- 각 문단은 \\n\\n으로 구분
- 문단 구성: ① 핵심 요약 리드문 ② 사건 배경 ③ 현재 상황 상세 ④ 시장 반응 및 수치 데이터 ⑤ 전문가/애널리스트 의견 인용 ⑥ 업계 반응 ⑦ 과거 사례 비교 ⑧ 투자자 시사점 ⑨ 향후 전망 ⑩ 마무리 및 주의사항
- 구체적인 수치(가격, 등락률, 거래량 등)를 반드시 포함
- 전문가 이름과 소속을 인용하여 신뢰도 확보
- 독자가 원문을 클릭하지 않아도 충분한 정보를 얻을 수 있도록 상세하게 작성
