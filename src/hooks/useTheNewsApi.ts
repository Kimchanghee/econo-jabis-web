...
const BLOCKED_DOMAINS = [
    "blog.naver.com", "blog.daum.net", "tistory.com", "blogspot.com",
    "medium.com", "cafe.naver.com", "blog.", "cafe.", "post.naver.com"
  ];
...
const detectCategory = (title: string, description: string): Category => {
    ...
    const keywords: Record<Category, string[]> = {
          "주식": ["stock", "equity", "shares", "market", "nasdaq", "s&p", "kospi", "주식", "증시", "ipo", "earnings", "dow jones", "wall street", "投資"],
          ...
          try {
                  ...
                  if (items.length > 0) {
                            const filtered = items.filter(
                                        (item) => !isBlockedSource(item.url, item.source) && !isBlockedTitle(item.title)
                                      );
